...
\begin{figure}[h!]
	\center
	\subfloat[\label{}]{%
	\includegraphics[scale=2.0]{figures/chapter5/cellular-grid/flower.png}
	}\hspace{40pt}%
	\subfloat[\label{}]{%	
	\includegraphics[scale=0.3]{figures/chapter5/cellular-grid/cellular-grid-illustration.pdf}
	}
...
Algorithm~\ref{alg:local-search} describes the local combinatorial process and Figure~\ref{fig:local-comb-square-results} presents the digital curve evolution when executing this algorithm for two different shapes with $n=50$.
...
\caption{Members of $\mathcal{N}_n$ for the square shape.}
\end{figure}
...
We obtain similar results for the previous experiments (see figure \ref{fig:simple-elastica-experiments}), except when handling the flower shape \sketch{It seems that is a matter of choosing the right estimation disk radius with respect to shape and grid step. Even for the digital elastica energy we may recover flows as described in this section if we use a disk radius that is not sensible enough in low curvature regions. As a result, we proportionally overestimate quite a lot points with low curvature. A better model would select the disk radius following some heuristic}. Flow evolution for smoother versions of the flower doesn't change this scenario, which suggests that the simplified flow is not suited for initial shapes with regions of negative curvature.
...
\begin{minipage}[c]{0.32\textwidth}
	\center
	\subfloat[\label{fig:fixed-c}]{
	\includegraphics[scale=0.19]{figures/chapter5/sqc/flow/flower/summary.pdf}}\\
	\subfloat[\label{fig:fixed-c}]{
...
Energy \eqref{eq:unconstrained-digital-elastica} is of order six and = we didn't try to optimize it.
