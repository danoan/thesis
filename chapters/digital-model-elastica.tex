...
\begin{algorithm}[H]
 ...
 \Input{A digital set $S$; the neighborhood size $n$ and its depth $K$; the maximum number of iterations \MIt; and a stop condition \Tol}
 \BlankLine
 \Delta $\longleftarrow$ \Tol+1\;
 \While{ \It $<$ \MIt \bf{and} \Delta $>$ \Tol  }{
   	\For{$ k \in [0,K] $}
   	{
  		\For{$ X \in \mathcal{N}_n(S^{(i)}_{+k}) \cup \mathcal{N}_n(S^{(i)}_{-k}) $}
		{
...
\section{Global Optimization Model}\label{sec:global-optmization-model}
