...
\label{ch6:sec:continuous-digital-elastica}
	...
	Given an Euclidean shape $X$ and parameters $\Theta=(\alpha,\beta) >0$. The digital Elastica $\hat{E}$ is defined as
	\begin{align}
...
\begin{algorithm}[H]
 ...
 \Input{A digital set $S$; weight coefficients $\Theta=(\alpha , \beta)$; the maximum number of iterations \MIt}
 \BlankLine
 $i \longleftarrow 0$\;	
 $S^{(0)} \longleftarrow S$\;
 \While{ $k < \log_2{|\partial S^{(i)}|}$ \bf{and} \It $<$ \MIt }
 {
	...
	{
		\If{ $\hat{E}_{\Theta}(X)$ $<$ $\hat{E}_{\Theta}(X^\star)$ }
		{
			$X^\star \longleftarrow X$\; 
		}	
	}
	\Delta $\longleftarrow$ $\hat{E}_{\Theta}(S^{(i-1)}) - \hat{E}_{\Theta}(S^{(i)})$\;	
...
The simplified digital Elastica is defined as
	...
	\begin{align}
	\hat{E}_{\Theta-sim}( D_h(X) ) = \sum_{\dot{\vec{e}} \in \partial D_h(X)}{ \alpha + \beta \hat{\kappa}_{r}^2(D_h(X),\dot{\vec{e}},h) }.
...
\begin{align}
	E_{\Theta-sim}(x,y) =& \sum_{y_i \in y}{ y_i \left(\; \alpha + \beta \hat{\kappa}_{r}^2(D,y_i) \; \right)}\\\nonumber
...
\begin{align*}
	\min_{x \in \mathbb{B}^{|X|}, y \in \mathbb{B}^{|Y|}}{E_{\Theta-sim}(x,y)}, \quad \text{subject to } T(x,y). \quad (P0)
\end{align*}
...
\begin{align*}
	\min_{x \in \mathbb{B}^{|X|}, y \in \mathbb{B}^{|Y|}}{E_{\Theta-sim}(x,y)}, \quad \text{subject to } T(x,y), R(x) \quad (P1)
\end{align*}
