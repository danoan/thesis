...
\label{ch5:sec:continuous-digital-elastica}
	The elastica energy of parameters $\vec{\theta}=(\alpha \geq 0, \beta \geq 0)$ for some Euclidean shape $S \subset \mathbb{R}^2$ is defined as
	\begin{align*}
...
sequence $\Ds^{(k)}$ of shapes with non-increasing elastica energy. The
idea is to define a neighborhood of shapes $\mathcal{W}^{(k)}$ to the
shape $\Ds^{(k)}$ and choose the element of $\mathcal{W}^{(k)}$ with
lowest energy.  
...
\end{align*}
\end{definition}
\daniel{Thus, the $m$-ring is composed of two sets of pixels with positive and negative distances to the shape $D$ if $m > 0$~(see~\cref{ch5:fig:ring-set}), and equal to the inner pixel boundary in the case $m=0$.} Consider the following set of neighbor candidates to $\Ds$:
\begin{align*}
\{ Q \; | \; Q \subset \left( R_1(\Ds) \cup \Ds \right) \; \text{and} \; \text{$Q$ is connected} \}.
\end{align*}
...
\begin{definition}{$n$-neighborhood}
	...
	Given a digital shape $\Ds \subset \Omega$, its $n$-neigh\-bor\-hood $\mathcal{W}_n(\Ds)$ is defined as the set of digital shapes that can be built from $\Ds$ by adding or removing a sequence of $n$ connected pixels in $R_1(\Ds)$.
...
\begin{algorithm}
 ...
 \Input{A digital set $\Ds$; weight coefficients $\vec{\theta}=(\alpha , \beta)$; the maximum number of iterations \MIt}
 \BlankLine
...
%	$\displaystyle \mathcal{N}^{(k,t)} \longleftarrow \displaystyle \Cup_{J}{\mathcal{N}_{j}(\Ds^{(k)})}$\;
	...
	$\displaystyle \mathcal{W}^{(k,t)} \longleftarrow \displaystyle \mathcal{W}_{n}(\Ds^{(k)})$\;
	
	\BlankLine
	
	\comment{Find neighbor shape with lowest energy.} 
	$Q^{\star} \longleftarrow D^{(k-1)}$\;
  	\For{$ Q \in \mathcal{W}^{(k,t)} $}
	{
		\If{ $\hat{E}_{\vec{\theta}}(Q)$ $<$ $\hat{E}_{\vec{\theta}}(Q^\star)$ }
		{
	...
	\Delta $\longleftarrow$ $\hat{E}_{\vec{\theta}}(\Ds^{(k-1)}) - \hat{E}_{\vec{\theta}}(\Ds^{(k)})$\;	
...
We study the behavior of~\cref{ch5:alg:local-search} in two problem configurations. 
...
\daniel{
As observed in~\cref{ch3:sec:elastica-curve}, the closed planar curve that minimizes the free elastica is the circumference of a disk of radius $(\beta/\alpha)^{1/2}$
\begin{align*}
	\partial B_{(\beta/\alpha)^{1/2}} = \argmin_C \int_{C}{ (\alpha + \beta \kappa^2)ds}.
\end{align*}
}
...
We have executed the same experiments for different parameters $\alpha$ to confirm the effectiveness of our approach. We observe that the plots for $\alpha=0.001$ in~\cref{fig:local-comb-estimators-plots-lp0001} follows a pattern similar to those in~\cref{fig:local-comb-estimators-plots-lp001} for $\alpha=0.01$. In particular, the remarks for the II-$3$ and MDCA estimator are the same. Further, we point out that II-$5$ values are slightly farther from the optimum for $\alpha=0.001$. The reason being that the shapes evolve to a disk of higher radius compared to the case $\alpha=0.01$. At some point of the evolution for $\alpha=0.001$, the sensibility of II-$5$ is not sufficient to escape from local minimum. We remark that the adoption of an automatic selection of the estimation disk radius may attenuate this problem.
...
\end{tabular}
\end{minipage}
\caption{\daniel{\textbf{MDCA sensitivity to noise.}} The ratio of change in energy value $(\Delta E)$ by change of the number of pixels ($\Delta D$) is much higher for the MDCA than for the II estimator. }
...
In contrast with the previous section, the model described here is designed for the integral invariant estimator only. Let $\Ds \subset \left( \mathbb{Z} + \frac{1}{2} \right)^2$ be the digitization of some shape $S \subset \mathbb{R}^2$ %using grid step $h$
 in half-integer coordinates space. We assume that $\Ds$ has $m$ pixels (located at integer coordinates denoted $p_j$) and $n$ linels (one and only one of its coordinates is $\frac{1}{2}$ denoted $\ell_i$). Optimization variables are represented as column vectors $\vec{x} \in \daniel{\{0,1\}}^{m},\, \vec{y} \in \daniel{\{0,1\}}^{n}$ and its $i$-th coefficients are denoted  $\vec{x}_i,\vec{y}_i$.  Further, let $\vec{A} \in \daniel{\{0,1\}}^{m\times n}$ the matrix defined as
\begin{align*}
...
\end{align*}
%
In other words, the column vector $\vec{A}_i$ represents the pixels that are in the interior of the disk $B_{r}(\ell_i)$ of radius $r$ centered at $\vec{y}_i$. 
\begin{align}
	E_{\vec{\theta}}^{simp}(\vec{x},\vec{y}) =& \sum_{\vec{y}_i \in \vec{y}}{ \vec{y}_i \left(\; \alpha + \beta \hat{\kappa}_{r}^2(D,\vec{y}_i) \; \right)}\\\nonumber
			   =& \sum_{\vec{y}_i \in \vec{y}}{ \vec{y}_i \left(\; \alpha  + \beta \big( \frac{3}{r^3}(\frac{\pi}{r^2} - |B_r(\ell_i)|)\big)^2\right)}\\\nonumber
...
\begin{align*}
	\min_{\vec{x} \in \daniel{\{0,1\}}^{m}, \vec{y} \in \daniel{\{0,1\}}^{n}}{E_{\vec{\theta}}^{simp}(\vec{x},\vec{y})}, \quad \text{subject to } T(\vec{x},\vec{y}). \quad (P0)
\end{align*}
%
...
\begin{align*}
	\min_{\vec{x} \in \daniel{\{0,1\}}^{m}, \vec{y} \in \daniel{\{0,1\}}^{n}}{E_{\vec{\theta}}^{simp}(\vec{x},\vec{y})}, \quad \text{subject to } T(\vec{x},\vec{y}), R(\vec{x}) \quad (P1)
\end{align*}
%
	Formulation $P1$ is a constrained binary non-convex third order problem and likely difficult to be solved optimally. Nonetheless, we can use standard optimization techniques in the hope to obtain a good approximation.
	
\subsection{Topological constraints}
\label{ch5:subsec:topological-constraints}

The estimation disk should be applied in the \daniel{digital contour} of the shape, which obliges us to impose topological constraints in the model to avoid inconsistent solutions. In order to accomplish that, we set an arbitrary orientation for the faces and another for the edges. We choose counter-clockwise for faces; left-to-right for horizontal edges; and bottom-to-up for vertical edges. \daniel{The topological constraints are essentially the same as those implemented in the linear programming model of~\cite{schoenemann09linear} and described in~\cref{ch3:sec:discrete-methods-squared-curvature}.}
...
%
\daniel{
For example, let $\vec{u}_i$ be the auxiliar variable associated with the opt-term $\vec{z}_1\vec{x}_2\vec{x}_4$. Then, we are going to add the following constraints
\begin{align*}
...
We can use the pixel incidence matrix defined in~\cref{app2:pixel-incidence-matrix} to define an unconstrained version of P1. The pixel incidence vector $\vec{q} \in \mathbb{Z}^m$ for pixels $\vec{x} \in \{0,1\}^{m}$ is 
	\begin{align*}
		\vec{q} &= \vec{P}^T\vec{P} \vec{x}
	\end{align*}
%
In order to suppress the sign, we define diagonal matrix $\vec{Q} \in \mathbb{R}^{m \times m }$ as
\begin{align*}
	\vec{Q} = diag(\vec{q})diag(\vec{q})
\end{align*}
%
Let $\vec{B} \in \{0,1\}^{m\times m}$ such that column vector $\vec{B}_j$ represents the pixels in the interior of a disk of radius $R$ centered at pixel $j$. Finally, we search for solutions of
\begin{align}
...
%
%
where $\mathbf{1} = (1,1, \cdots , 1)^T \in \mathbb{R}^m$. ~\cref{eq:unconstrained-digital-elastica} involves the minimization of a fourth order equation and is therefore hard to optimize.
