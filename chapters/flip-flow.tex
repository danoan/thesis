...
\begin{align}
  E_{m-flip}(X^{(k)},S^{(k)}) =& \sum_{x_j \in X^{(k)}}{\alpha s(x_j)} + \nonumber \\ 
  ...
  & \sum_{x_j \in X^{(k)}}{\gamma g(S^{(k)},x_j)},
...
\begin{algorithm}
 ...
 \While{ \It $<$ \MIt  }{ 	
	...
	\If{ \It is even }{	 
	 	$x^{(k)} \longleftarrow \argmin_{X^{(k)}} E_{m-flip}(1-X^{(k)},S^{(k-1)})$\; 	
 		$S^{(k)} \longleftarrow F^{(k-1)} + x^{(k)}$\;
 	}
	\comment{Expansion mode} 	
 	\Else{	
	 	$x^{(k)} \longleftarrow \argmin_{X^{(k)}} E_{m-flip}(1-X^{(k)},\overline{S^{(k-1)}})$\;
 		$S^{(k)} \longleftarrow \overline{ F^{(k-1)} + x^{(k)}}$\; 	
 	}
...
\begin{align*}
	S^{(k+1)} = F_{S}^{(k)} + \argmin _{}{E_{m-flip}\big(1-X^{(k)},S^{(k)}} \big).
\end{align*}
...
\begin{table}
  ...
  \begin{tabular}{|c|c|c|c|} \hline
    ...
    $S^{(k+1)} \leftarrow S^{(k)} - x^{(k)}$ & eroded & prob. eroded & unchanged  \\ \hline \hline
    expansion mode &    $\bar{\kappa} \gg 0$ & $\bar{\kappa} \geq 0$ & $\bar{\kappa} < 0$ \\ \hline
    $\bar{x}^{(k)}$ & $\bar{x}_j=1$ & $\bar{x}_j \in \{0,1\}$ & $\bar{x}_j=0$ \\ \hline
    $S^{(k+1)} \leftarrow \overline{\bar{S}^{(k)} - \bar{x}^{(k)}}$ & dilated & prob. dilated & unchanged \\ \hline 
  \end{tabular}
...
\begin{align}
  g(x_{w(p)}) = -(1-x_{w(p)})\log{H_f(p)} - x_{w(p)}\log{H_b(p)}.
...
\begin{algorithm}
 ...
 \While{ \It $<$ \MIt \bf{and} \Delta $>$ \Tol  }{ 	
 	$S^{(k+1)} \longleftarrow $ FlipFlow($S^{(k)},r,\alpha,\beta,\gamma,2$)\;
