...
\begin{algorithm}
 ...
 \While{ \It $<$ \MIt  }{ 	
	...
	\If{ \It is even }{	 
	 	$x^{(k-1)} \longleftarrow \argmin_{X^{(k-1)}} E_{m-flip}(1-X^{(k-1)},S^{(k-1)})$\; 	
 		$S^{(k)} \longleftarrow F^{(k-1)} + x^{(k-1)}$\;
 	}
	\comment{Expansion mode} 	
 	\Else{	
	 	$x^{(k-1)} \longleftarrow \argmin_{X^{(k-1)}} E_{m-flip}(1-X^{(k-1)},\overline{S^{(k-1)}})$\;
 		$S^{(k)} \longleftarrow \overline{ F^{(k-1)} + x^{(k-1)}}$\; 	
 	}
